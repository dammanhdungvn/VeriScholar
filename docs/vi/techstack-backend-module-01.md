...
| **Relational & Vector DB** *(CSDL quan hệ & vector)* | **PostgreSQL 16** + **`pgvector`** | Lưu siêu dữ liệu bài báo (Metadata), các đoạn văn bản (Chunks) và vector nhúng (HNSW Index `vector_cosine_ops`). | Hợp nhất dữ liệu quan hệ và dữ liệu vector trong 1 cơ sở dữ liệu duy nhất hỗ trợ giao dịch ACID, loại bỏ hoàn toàn rủi ro lệch dữ liệu so với việc dùng Vector DB chuyên biệt ngoài. |
